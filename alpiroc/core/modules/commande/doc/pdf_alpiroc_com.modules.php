 ...
 */
class pdf_alpiroc_com extends ModelePDFCommandes
{
	var $db;
	var $name;
	var $description;
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		$alpiroc->fetch_selectprofil($object->id,"commande");//Sélectionne le nom du profil choisi 
		...
		if ($this->profil!=''){
			...
			$this->option_sign_aera=0;//Toujours désactivé sur une commande
		...
		//Une variable global non défini existe pour ne pas répéter l'header, on l'utilise. 
			...
			if (file_exists($dir))
			{
				...
				if ($this->option_head=="alpiroc"){
					...
					if ($this->option_rappel==1){
						...
						$pdf->MultiCell(100, 4, $outputlangs->transnoentities("Order"), 0, 'L');
					}
					$posy=$pdf->GetY();
					$nexY=$posy;
					 ...
					 if ($object->ref_client){//On ecrit de la même manière les refs clients
						...
						$pdf->MultiCell(70, 4, $outputlangs->transnoentities("OrderDate")." : " . dol_print_date($object->date,"day",false,$outputlangs,true), '', 'R');
						$nexY=max($nexY,$pdf->GetY());
						...
						$nexY=max($nexY,$pdf->GetY());
						$pdf->SetXY($this->page_largeur-$this->marge_droite-100,$posy);
						$pdf->MultiCell(70, 4, $outputlangs->transnoentities("OrderDate")." : " . dol_print_date($object->date,"day",false,$outputlangs,true), '', 'L');
						$nexY=max($nexY,$pdf->GetY());
					 }
			...
			if ($this->option_rappel==0){
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					//Insert d'un Titre
					$alpiroc=new Alpiroc($this->db);
					$alpiroc->fetch_titre($object->lines[$i]->rowid,"titrealpiroc","commandedet");
					if ($alpiroc->name!=''){
						$pdf->SetXY($curX,$curY);
						$pdf->SetTextColor(0,0,0);
						$pdf->SetFont('','B', $default_font_size +2);
					...
					$alpiroc=new Alpiroc($this->db);
					$alpiroc->fetch_titre($object->lines[$i+1]->rowid,"titrealpiroc","commandedet");//next ligne à un titre ?
				...
				if ($this->option_rappel==1){
					...
					$posy2=0;
					if ($this->option_condreg!=0 OR $this->option_modereg!=0){
						// Affiche zone infos
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		//~ if ($this->option_acompte==1 AND $this->option_rappel==0){
		...
		if (empty($conf->global->COMMANDE_PDF_HIDE_PAYMENTTERMCOND) && ($object->cond_reglement_code || $object->cond_reglement))
		{
		...
		if (empty($conf->global->COMMANDE_PDF_HIDE_PAYMENTTERMCOND))
		{
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			else
			{
				...
				$deja_paye=0;
				if ($this->option_acompte==2 AND $this->option_rappel==1){
					//Récupère la valeur de l'acompte
					$alpiroc=new Alpiroc($this->db);
					$alpiroc->fetchValueFromProfil("acompte",$this->profil);
					$acompte=$alpiroc->content;
	...
	function _pagehead_azur(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
{
		...
		if ($showaddress)
		{
			...
			//~ $posy_note_private=$posy+42;			
			//~ $posy=$posy_note_private+5;
