 ...
 */
class pdf_alpiroc extends ModelePDFPropales
{
	var $db;
	var $name;
	var $description;
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		$alpiroc->fetch_selectprofil($object->id,"propal");//Sélectionne le nom du profil choisi 
		...
		//Une variable global non défini existe pour ne pas répéter l'header, on l'utilise. 
			...
			if (file_exists($dir))
			{
				...
				if ($this->option_head=="alpiroc"){
					...
					if ($this->option_rappel==1){
						...
						$pdf->MultiCell(100, 4, $outputlangs->transnoentities("CommercialProposal"), 0, 'L');
					}
					$posy=$pdf->GetY();
					$nexY=$posy;
					 ...
					 if ($object->ref_client){//On ecrit de la même manière les refs clients
						...
						$pdf->MultiCell(70, 4, $outputlangs->transnoentities("DatePropal")." : " . dol_print_date($object->date,"day",false,$outputlangs,true), '', 'R');
						$nexY=max($nexY,$pdf->GetY());
						...
						$nexY=max($nexY,$pdf->GetY());
						$pdf->SetXY($this->page_largeur-$this->marge_droite-100,$posy);
						$pdf->MultiCell(70, 4, $outputlangs->transnoentities("DatePropal")." : " . dol_print_date($object->date,"day",false,$outputlangs,true), '', 'L');
						$nexY=max($nexY,$pdf->GetY());
					 }
			...
			if ($this->option_rappel==0){
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					//Insert d'un Titre
					$alpiroc=new Alpiroc($this->db);
					$alpiroc->fetch_titre($object->lines[$i]->rowid,"titrealpiroc","propaldet");
					if ($alpiroc->name!=''){
						$pdf->SetXY($curX,$curY);
						$pdf->SetTextColor(0,0,0);
						$pdf->SetFont('','B', $default_font_size +2);
					...
					$alpiroc=new Alpiroc($this->db);
					$alpiroc->fetch_titre($object->lines[$i+1]->rowid,"titrealpiroc","propaldet");//next ligne à un titre ?
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($this->option_acompte==1 AND $this->option_rappel==0){
			...
			$pdf->SetXY($posxval, $posy);
			$pdf->SetFont('','', $default_font_size - 2);
			$pdf->MultiCell(50, 4, $texte, 0, 'L');//Multicell de 50 au lieu de 65 pour faire passer le texte sur 2 lignes
			$posy=$pdf->GetY()+1;
		}
